...
<p><?php printf( __( 'Email ini memberitahu bahwa %s telah melakukan order ke website Astronacci. Berikut ini detailnya:', WC_Subscriptions::$text_domain ), $order->billing_first_name . ' ' . $order->billing_last_name ); ?></p>

<?php do_action( 'woocommerce_email_before_order_table', $order, true ); ?>

<h2><?php printf( __( 'Order: %s', WC_Subscriptions::$text_domain), $order->get_order_number() ); ?> (<?php printf( '<time datetime="%s">%s</time>', date_i18n( 'c', strtotime( $order->order_date ) ), date_i18n( woocommerce_date_format(), strtotime( $order->order_date ) ) ); ?>)</h2>
